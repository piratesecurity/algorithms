// Topological Sorting program for graph using C Language
// Status Color g:Grey, w:White , b:Black
// I=infinity (I just take it as -1. What ever you can take it works.)
// Time Complexity O(|V|+|E|).
// It returns the decresing order of finishing times of vertices in DFS.
...
void main()
	{
		...
		node * topo_order=topo_sort;
		while (topo_order)
			{
				printf("%d ",topo_order->value);
...
void VISIT_NODE(int current_node)
	{
		...
		node * new_node= (node * )malloc(sizeof(node *));
		new_node->value=current_node;
		new_node->next=topo_sort;
		topo_sort=new_node;
